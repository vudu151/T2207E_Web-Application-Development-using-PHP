<?php
    ...
    function getListNV($maphong=0, $hoten="", $dienthoai="")   //Tham số $maphong mặc định = 0(khi không có tham số)
    {
        $conn = ConnectDB();
        if($conn==null)
            return null;
        ...
        $sql = "SELECT nv.*,pb.tenphong FROM tbNhanvien nv INNER JOIN tbPhongban pb ON nv.maphong=pb.maphong WHERE TRUE ";
        ...
        if($dienthoai!="")
            $sql .= " AND nv.dienthoai = '$dienthoai' ";
